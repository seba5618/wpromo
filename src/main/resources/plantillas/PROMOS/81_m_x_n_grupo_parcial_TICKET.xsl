...
<xsl:template match="promo[@promotype='m_x_n_grupo_parcial_TICKET']">
	...
	<xsl:variable name="promo_name">Promoción: M x N grupo parcial en ticket</xsl:variable>
...
//********** Promoción Invel: M x N grupo parcial en ticket *************

Promotion <xsl:value-of select="$eval"/> m_x_n_grupo_parcial_TICKET_<xsl:value-of select="$cod_promo"/>
...
<xsl:value-of select="$condiciones"/>
	requireAny{<xsl:for-each select="articulos/item">(<xsl:apply-templates select="element"/>,1)<xsl:if test="not(position()=last())">, </xsl:if></xsl:for-each>};
...
<!-- >>>>>>>>>>>>>> -->
	(<xsl:for-each select="articulos/item">(\arts_a_bonificar_<xsl:number value="position()"/>\ ) <xsl:if test="not(position()=last())"> + 
	</xsl:if></xsl:for-each>) &gt;= por_cada_arts;  
...
<!-- <<<<<<<<<&gt;= por_cada_arts;<<<<< -->
<!--  <xsl:for-each select="articulos/item"> -->
	blog(file, "Promo LLEGAMOS"++ <xsl:value-of select="$promo_id"/> ++ ": LATENTE");
	contador = <xsl:for-each select="articulos/item">\arts_a_bonificar_<xsl:number value="position()"/>\ <xsl:if test="not(position()=last())"> + </xsl:if></xsl:for-each>;
	a_1 = (( contador  / por_cada_arts) * bonifico_arts);
	continuo = 1;
	if a_1 &gt; 0 then
		blog(file, "Promo SUPERAMOS"++ <xsl:value-of select="$promo_id"/> ++ ": LATENTE");
		<xsl:for-each select="articulos/item">
<!--			c_<xsl:number value="position()"/> = \arts_a_bonificar_<xsl:number value="position()"/>\ ; -->
			...
			if c_<xsl:number value="position()"/> &gt; 0 then
				 blog(file, "Promo vamosentrando"++ <xsl:value-of select="$promo_id"/> ++ ": LATENTE");
				if( continuo == 1) then
					blog(file, "Promo entre 1-"++ <xsl:number value="position()"/> ++ ": LATENTE");
			<!--				publicar_<xsl:value-of select="$index"/> = publicar_<xsl:value-of select="$index"/> + {(art_<xsl:number value="position()"/>, a_1 * ${(art_<xsl:number value="position()"/>,1)} * descuento)};-->
				...
				<!--	publicar_arts_<xsl:value-of select="$index"/> = publicar_arts_<xsl:value-of select="$index"/> + {(art_<xsl:number value="position()"/>, a_1)};  -->
...
<!-- <<<<<<<<<<<<<< -->
	// Parametros correspondientes a la promo m_x_n_grupo_parcial_TICKET_<xsl:value-of select="$cod_promo"/>
<xsl:if test="$eans='uno'">
	...
	// -------------------------------- promo m_x_n_grupo_parcial_TICKET_<xsl:value-of select="$cod_promo"/>
<!-- >>>>>>>>>>>>>> -->
</xsl:if>
...
<!-- <<<<<<<<<<<<<< -->
	// Aplicando Beneficios correspondientes a la promo m_x_n_grupo_parcial_TICKET_<xsl:value-of select="$cod_promo"/>
