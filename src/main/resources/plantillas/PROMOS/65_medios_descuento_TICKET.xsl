...
<xsl:template match="promo[@promotype='medios_descuento_ticket']">
...
Promotion <xsl:value-of select="$eval"/> medios_descuento_ticket_<xsl:value-of select="$cod_promo"/>
...
<xsl:if test="mensaje_medios/mensaje/linea">
  ...
  a_pagar = 0;
  rta = input("Pagara con alguno de esos medios? (ENTER=SI / ESC=No)");
  
  if rta then
    info_monto = " Hay $" ++ monto ++ " en articulos promocionados.&amp;";
    a_pagar = numinput(info_monto ++ "Monto a pagar:");
    if a_pagar == 0 then
  ...
  if a_pagar > 0 then
    descuento_ajustado = <xsl:value-of select="$descuento"/> * ( (monto min a_pagar) / (monto + <xsl:value-of select="$monto_benef"/>) );
    <xsl:value-of select="$monto_benef"/> = <xsl:value-of select="$monto_benef"/> + (monto min a_pagar);
		...
		a_descontar = \publicar_<xsl:value-of select="$index"/>\;
		
		<xsl:value-of select="$beneficiados"/> = <xsl:value-of select="$beneficiados"/> + publicar_arts_<xsl:value-of select="$index"/>;
		<xsl:value-of select="$benef"/> = a_descontar * <xsl:value-of select="$cash2benef"/>;
...
<!-- <<<<<<<<<<<<<< -->
	// Aplicando Beneficios correspondientes a la promo medios_descuento_ticket_<xsl:value-of select="$cod_promo"/>
...
<xsl:if test="$msj_ticket='true'">
	...
	// ------------------------------------------ promo medios_descuento_ticket_<xsl:value-of select="$cod_promo"/>
