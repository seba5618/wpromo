...
<!--***********************************************************************
	...
	Autor: Fabian Ramirez //Marcelo mancuso
	Fecha: 21/07/2009 // 11/01/2017 para uaa
  **********************************************************************-->

<xsl:template match="promo[@promotype='depto_plan_pago_descuento_ticket']">
...
<!-- <<<<<<<<<<<<<< -->
	...
	<xsl:variable name="medios_requeridos">{<xsl:for-each select="medios/m/medio">
		...
		<xsl:variable name="mediorequerido"><xsl:value-of select="medios/m/medio"/>
	...
	<xsl:variable name="submedio_requerido"><xsl:value-of select="medios/m/submedio"/>
	...
	<xsl:variable name="plan_requerido"><xsl:value-of select="medios/m/plan"/>
...
Promotion <xsl:value-of select="$eval"/> depto_plan_pago_descuento_ticket_<xsl:value-of select="$cod_promo"/>
...
<xsl:if test="$ignora_exclusion_arts='true'">
	bolson = (buyed - nopromotion) meet 0.001 * deptos;
</xsl:if>
<xsl:if test="$ignora_exclusion_arts='false'">
	bolson = purchase meet 0.001 * deptos;
</xsl:if>
</xsl:if>

<xsl:if test="$comportamiento='articulos'">
<xsl:if test="$ignora_exclusion_arts='true'">
	bolson = (purchase - nopromotion) meet 0.001 * deptos;
</xsl:if>
<xsl:if test="$ignora_exclusion_arts='false'">
	bolson = purchase  meet 0.001 * deptos;;
</xsl:if>
...
<xsl:if test="mensaje_medios/mensaje/linea">
  ...
  a_pagar = 0;
  <!-- info_monto = monto; -->
  info_monto = numRNV( ram_total ) - $excluded ;
  monto = numRNV( ram_resto ) - $excluded ;
  a_pagar = info_monto;
  
  if a_pagar > 0 then
    descuento_ajustado = <xsl:value-of select="$descuento"/> * ( (monto min a_pagar) / (monto + 0) );
	descuento_ajustado2 = descuento_ajustado * 100;
   <!-- <xsl:value-of select="$monto_benef"/> = 0 + (monto min a_pagar);-->
     <!-- a_pagar2 = numinput(info_monto2 ++ "IMPORTE A COBRAR:");-->
	...
	publicar_arts_<xsl:value-of select="$index"/> = bolson  ;
		...
		<!--info_monto3 = "Precio Final " ++ (monto -( limite_entrega min (a_descontar  * 1))) ++ "   " ++ ".&amp;"  ++  "Aplico Desc/Recar " ++ ( limite_entrega min (a_descontar  * 1)) ++  ".&amp;";-->
		info_monto3 = "Precio Final " ++ (monto -( limite_entrega min (a_descontar  * 1))) ++ "   " ++ "|~CON CUANTO ABONA ?: " ;
		
		
		<!--Precio Final 70.00 |~CON CUANTO ABONA ?:|Aplico Desc/Recar 30.00.   -->
		<!--Simbolo | indica salto de linea y  ~ input 
		a_pagar = numinput(info_monto3 ++ "|Aplico Desc/Recar " ++ ( limite_entrega min (a_descontar  * 1)));-->
		a_pagar = numinput(info_monto3 ++ "|Aplico Desc/Recar " ++ ( $excluded));
		
		if a_pagar == 0 then
			a_pagar = $wholepurchase;
		<!--	-->
		fi;
	...
	<!--monto_requerido_<xsl:value-of select="$index"/> = (monto min a_pagar); -->
	monto_requerido_<xsl:value-of select="$index"/> = a_pagar; 
	<!--	monto_requerido_<xsl:value-of select="$index"/> = (monto min a_pagar) - a_descontar; -->
<!-- origial		monto_requerido_<xsl:value-of select="$index"/> = a_pagar ; -->
...
<!-- <<<<<<<<<<<<<< -->
	...
	// -------------------------------- promo depto_plan_pago_descuento_ticket_<xsl:value-of select="$cod_promo"/>
...
<!-- <<<<<<<<<<<<<< -->
	// Aplicando Beneficios correspondientes a la promodepto_plan_pago_descuento_ticket_<xsl:value-of select="$cod_promo"/>
...
requireMeans(medios_requeridos_<xsl:value-of select="$index"/>, monto_requerido_<xsl:value-of select="$index"/>);
<!--  original>>>>>>>>>>>>>> 	requireMeans(medios_requeridos_<xsl:value-of select="$index"/>, monto_requerido_<xsl:value-of select="$index"/> min $wholepurchase );-->
