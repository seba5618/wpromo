...
<xsl:template match="promo[@promotype='articulo_plan_pago']">
	...
	<xsl:variable name="promo_name">Promoción: Artículo plan de pago</xsl:variable>
...
Promotion <xsl:value-of select="$eval"/> articulo_plan_pago_<xsl:value-of select="$cod_promo"/>
...
<!-- <<<<<<<<<<<<<< -->
	...
	for (a,k) in arts_a_bonificar do
		bolson_a = bolson.a;
		cantidad_<xsl:value-of select="$index"/> = cantidad_<xsl:value-of select="$index"/> + &amp;(k/bolson_a) * ${(a,bolson_a)};
...
Promotion postpago articulo_plan_pago_LOGS_<xsl:value-of select="$cod_promo"/>
...
<!-- <<<<<<<<<<<<<< -->
	// Parametros correspondientes a la promo articulo_plan_pago_<xsl:value-of select="$cod_promo"/>
	extern cantidad_<xsl:value-of select="$index"/>;
	extern publicar_arts_<xsl:value-of select="$index"/>;

	extern promo_activada_<xsl:value-of select="$index"/>;
	// -------------------------------- promo articulo_plan_pago_<xsl:value-of select="$cod_promo"/>
<!-- >>>>>>>>>>>>>> -->
</xsl:if>
...
<!-- <<<<<<<<<<<<<< -->
	// Aplicando Beneficios correspondientes a la promo articulo_plan_pago_<xsl:value-of select="$cod_promo"/>
