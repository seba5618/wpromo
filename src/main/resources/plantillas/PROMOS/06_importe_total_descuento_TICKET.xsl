...
<!--***********************************************************************
	Promoción: Importe total descuento en ticket
	Autor: Elio Mauro Carreras
	Fecha: 12/03/2007
  **********************************************************************-->

<xsl:template match="promo[@promotype='importe_total_descuento_tkt']">
	...
	<xsl:variable name="promo_name">Promoción: Importe total descuento en ticket</xsl:variable>
...
//********** Promoción Invel: Importe Total Descuento en Ticket *************

Promotion <xsl:value-of select="$eval"/> importe_total_descuento_ticket_<xsl:value-of select="$cod_promo"/>
...
<xsl:if test="$ignora_exclusion_arts='true'">
	monto  = $(buyed - nopromotion <xsl:if test="$excluir_articulos_precio_oferta='true'">- department specialCreditPrice</xsl:if>) - \<xsl:value-of select="$beneficiados"/>\;
</xsl:if>
<xsl:if test="$ignora_exclusion_arts='false'">
	monto  = $(purchase <xsl:if test="$excluir_articulos_precio_oferta='true'">- department specialCreditPrice</xsl:if>) - \<xsl:value-of select="$beneficiados"/>\;
</xsl:if>
</xsl:if>
...
<!-- <<<<<<<<<<<<<< -->
	valor = ((monto * descuento) min limite_entrega);
<xsl:if test="$ignora_exclusion_arts='true'">
	p_buyed = $(buyed - nopromotion <xsl:if test="$excluir_articulos_precio_oferta='true'">- department specialCreditPrice</xsl:if>);
	for (a,k) in (buyed - nopromotion <xsl:if test="$excluir_articulos_precio_oferta='true'">- department specialCreditPrice</xsl:if>) do
		publicar_<xsl:value-of select="$index"/> = publicar_<xsl:value-of select="$index"/> + {(a, valor * ${(a,k)} / p_buyed)};
	od;
...
<xsl:if test="$ignora_exclusion_arts='false'">
	p_purch = $(purchase <xsl:if test="$excluir_articulos_precio_oferta='true'">- department specialCreditPrice</xsl:if>);
	for (a,k) in (purchase <xsl:if test="$excluir_articulos_precio_oferta='true'">- department specialCreditPrice</xsl:if>) do
		publicar_<xsl:value-of select="$index"/> = publicar_<xsl:value-of select="$index"/> + {(a, valor * ${(a,k)} / p_purch)};
	od;
...
<!-- <<<<<<<<<<<<<< -->
	// Parametros correspondientes a la promo importe_total_descuento_ticket_<xsl:value-of select="$cod_promo"/>
	...
	// -------------------------------- promo importe_total_descuento_ticket_<xsl:value-of select="$cod_promo"/>
<!-- >>>>>>>>>>>>>> -->
</xsl:if>
...
<!-- <<<<<<<<<<<<<< -->
	// Aplicando Beneficios correspondientes a la promo importe_total_descuento_ticket_<xsl:value-of select="$cod_promo"/>
...
<!-- <<<<<<<<<<<<<< -->
	valor = ((&amp;(monto / por_cada) * descuento_fijo) min limite_entrega);
<xsl:if test="$ignora_exclusion_arts='true'">
	p_buyed = $(buyed - nopromotion <xsl:if test="$excluir_articulos_precio_oferta='true'">- department specialCreditPrice</xsl:if>);
	for (a,k) in (buyed - nopromotion <xsl:if test="$excluir_articulos_precio_oferta='true'">- department specialCreditPrice</xsl:if>) do
		publicar_<xsl:value-of select="$index"/> = publicar_<xsl:value-of select="$index"/> + {(a, valor * ${(a,k)} / p_buyed)};
