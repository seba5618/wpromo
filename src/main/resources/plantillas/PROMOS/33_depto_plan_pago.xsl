...
<!--***********************************************************************
	Promoción: Departamento plan de pago
	Autor: Elio Mauro Carreras
	Fecha: 12/03/2007
  **********************************************************************-->

<xsl:template match="promo[@promotype='depto_plan_pago']">
	...
	<xsl:variable name="promo_name">Promoción: Departamento plan de pago</xsl:variable>
...
//********** Promoción Invel: Departamento Plan de Pago *************

Promotion <xsl:value-of select="$eval"/> depto_plan_pago_<xsl:value-of select="$cod_promo"/>
...
//********** Promoción Invel: Departamento Plan de Pago // Impresiones en el Ticket  *************

Promotion postpago depto_plan_pago_LOGS_<xsl:value-of select="$cod_promo"/>
...
<!-- <<<<<<<<<<<<<< -->
	// Parametros correspondientes a la promo depto_plan_pago_<xsl:value-of select="$cod_promo"/>
	extern cantidad_<xsl:value-of select="$index"/>;
	extern publicar_arts_<xsl:value-of select="$index"/>;

	extern promo_activada_<xsl:value-of select="$index"/>;
	// -------------------------------- promo depto_plan_pago_<xsl:value-of select="$cod_promo"/>
<!-- >>>>>>>>>>>>>> -->
</xsl:if>
...
<!-- <<<<<<<<<<<<<< -->
	// Aplicando Beneficios correspondientes a la promo depto_plan_pago_<xsl:value-of select="$cod_promo"/>
