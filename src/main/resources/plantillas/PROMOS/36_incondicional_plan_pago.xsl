...
<!--***********************************************************************
	Promoción: Incondicional plan de pago
	Autor: Elio Mauro Carreras
	Fecha: 16/03/2007
  **********************************************************************-->

<xsl:template match="promo[@promotype='incondicional_plan_pago']">
	...
	<xsl:variable name="promo_name">Promoción: Incondicional plan de pago</xsl:variable>
	<xsl:variable name="promo_version">1.8.4</xsl:variable>
...
//********** Promoción Invel: Incondicional Plan de Pago *************

Promotion <xsl:value-of select="$eval"/> incondicional_plan_pago_<xsl:value-of select="$cod_promo"/>
...
//********** Promoción Invel: Incondicional Plan de Pago // Impresiones en el Ticket  *************

Promotion postpago incondicional_plan_pago_LOGS_<xsl:value-of select="$cod_promo"/>
...
Benefits
	...
	if promo_activada_<xsl:value-of select="$index"/> then
		if (<xsl:for-each select="medio/card">means.medio<xsl:number value="position()"/>_<xsl:value-of select="$index"/> &gt; 0 <xsl:if test="not(position()=last())"> || </xsl:if></xsl:for-each>) then
...
<!-- <<<<<<<<<<<<<< -->
	// Parametros correspondientes a la promo incondicional_plan_pago_<xsl:value-of select="$cod_promo"/>
<xsl:for-each select="medio/card">
	extern medio<xsl:number value="position()"/>_<xsl:value-of select="$index"/>;
</xsl:for-each>
	...
	extern promo_activada_<xsl:value-of select="$index"/>;
	// -------------------------------- promo incondicional_plan_pago_<xsl:value-of select="$cod_promo"/>
<!-- >>>>>>>>>>>>>> -->
</xsl:if>
...
<!-- <<<<<<<<<<<<<< -->
	// Aplicando Beneficios correspondientes a la promo incondicional_plan_pago_<xsl:value-of select="$cod_promo"/>
...
<xsl:for-each select="medio/card">
	activateMean(medio<xsl:number value="position()"/>_<xsl:value-of select="$index"/>,cantidad_<xsl:value-of select="$index"/>);
	rec(artsBeneficiary, publicar_arts_<xsl:value-of select="$index"/>);
	rec(meanBenefit, medio<xsl:number value="position()"/>_<xsl:value-of select="$index"/>, cantidad_<xsl:value-of select="$index"/>);
</xsl:for-each>
