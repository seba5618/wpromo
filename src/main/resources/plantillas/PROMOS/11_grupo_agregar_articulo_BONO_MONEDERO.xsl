...
<!--***********************************************************************
	Promoción: Grupo agregar artículo en bono
	Autor: Elio Mauro Carreras
	Fecha: 12/03/2007
  **********************************************************************-->

<xsl:template match="promo[@promotype='grupo_agregar_articulo_bono']">
	...
	<xsl:variable name="promo_name">Promoción: Grupo agregar artículo en bono</xsl:variable>
...
//********** Promoción Invel: Grupo agregar artículo en Bono o Monedero *************

Promotion <xsl:value-of select="$eval"/> grupo_agregar_articulo_bono_monedero_<xsl:value-of select="$cod_promo"/>
...
<!-- <<<<<<<<<<<<<< -->
	...
	num_arts = &amp;(\c * bolson\ / por_cada_arts) * bonifico_arts;

	n = 0;
	conjunto = {};
	for (a,k) in c * bolson do
		if n &lt; num_arts then
			cant = k min (num_arts - n);
			conjunto = conjunto + {(a,cant)};
			n = n + cant;
		fi;
	od;

	valor = descuento * $conjunto;
	publicar_arts_<xsl:value-of select="$index"/> = conjunto;
...
<!-- <<<<<<<<<<<<<< -->
	// Parametros correspondientes a la promo grupo_agregar_articulo_bono_monedero_<xsl:value-of select="$cod_promo"/>
<xsl:if test="$destino='bono'">
...
<xsl:if test="$destino='monedero'">
	...
	// -------------------------------- promo grupo_agregar_articulo_bono_monedero_<xsl:value-of select="$cod_promo"/>
<!-- >>>>>>>>>>>>>> -->
</xsl:if>
...
<!-- <<<<<<<<<<<<<< -->
	// Aplicando Beneficios correspondientes a la promo grupo_agregar_articulo_bono_monedero_<xsl:value-of select="$cod_promo"/>
