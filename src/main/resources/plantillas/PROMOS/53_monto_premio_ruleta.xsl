...
<!--***********************************************************************
	Promoción: Monto Premio Ruleta
	Autor: Elio Mauro Carreras
	Fecha: 26/05/2008
  **********************************************************************-->

<xsl:template match="promo[@promotype='monto_premio_ruleta']">
	...
	<xsl:variable name="promo_name">Promoción: Monto Premio Ruleta</xsl:variable>
	<xsl:variable name="promo_version">1.0.2</xsl:variable>
	<xsl:variable name="promo_date">16/10/08</xsl:variable>
...
<!-- <<<<<<<<<<<<<< -->
	...
	<xsl:variable name="monto_min_compra"><xsl:apply-templates select="monto_min_compra"/></xsl:variable>
...
//********** Promoción Invel: Monto Premio Ruleta *************

Promotion <xsl:value-of select="$eval"/> monto_premio_ruleta_<xsl:value-of select="$cod_promo"/>
...
<!-- <<<<<<<<<<<<<< -->
	...
	monto_min_compra = <xsl:value-of select="$monto_min_compra"/>;
...
<xsl:if test="$descuento_al_total_bool='true'">
	...
	d_total_bool = true;
	descarga = <xsl:value-of select="$descarga"/>;
</xsl:if>
...
<!-- <<<<<<<<<<<<<< -->
	$arts_a_bonificar &gt;= monto_min_compra;
	num_premios &gt;= 1;
...
<xsl:if test="$descuento_al_total_bool='true'">
	if(sorteo == num_premios_arts + 1 &amp;&amp; d_total_bool)then
		publicar_<xsl:value-of select="$index"/> = publicar_<xsl:value-of select="$index"/> + {(descarga, $purchase * descuento_al_total)};
...
<!-- <<<<<<<<<<<<<< -->
	// Parametros correspondientes a la promo monto_premio_ruleta_<xsl:value-of select="$cod_promo"/>
	...
	// -------------------------------- promo monto_premio_ruleta_<xsl:value-of select="$cod_promo"/>
<!-- >>>>>>>>>>>>>> -->
</xsl:if>
...
<!-- <<<<<<<<<<<<<< -->
	// Aplicando Beneficios correspondientes a la promo monto_premio_ruleta_<xsl:value-of select="$cod_promo"/>
	...
	for (a,k) in publicar_<xsl:value-of select="$index"/> do
		...
		if(distribuir_<xsl:value-of select="$index"/>)then
     	...
     	credit(a, k, publicar_arts_<xsl:value-of select="$index"/>);
