...
<!--***********************************************************************
	Promoción: Incondicional plan de pago con deptos excluidos
	Autor: Elio Mauro Carreras
	Fecha: 12/03/2007
  **********************************************************************-->

<xsl:template match="promo[@promotype='incondicional_plan_pago_deptos_excluidos']">
	...
	<xsl:variable name="promo_name">Promoción: Incondicional plan de pago con deptos excluidos</xsl:variable>
	<xsl:variable name="promo_version">1.8.6</xsl:variable>
	<xsl:variable name="promo_date">05/08/08</xsl:variable>
...
<!-- <<<<<<<<<<<<<< -->
	...
	<xsl:variable name="medio"><xsl:apply-templates select="medio/card"/></xsl:variable>
	<xsl:variable name="deptos"><xsl:apply-templates select="deptos"/></xsl:variable>
	<xsl:variable name="deptos_excluidos"><xsl:apply-templates select="deptos_excluidos"/></xsl:variable>
...
//********** Promoción Invel: Incondicional Plan de Pago Deptos Excluidos *************

Promotion <xsl:value-of select="$eval"/> incondicional_plan_pago_deptos_excluidos_<xsl:value-of select="$cod_promo"/>
...
<xsl:if test="$deptos_excluidos='true'">
	arts_a_bonificar = (buyed - nopromotion) meet (0.00001 * !(<xsl:value-of select="$deptos"/>));
</xsl:if>
<xsl:if test="$deptos_excluidos='false'">
	arts_a_bonificar = (buyed - nopromotion);
</xsl:if>
</xsl:if>
<xsl:if test="$ignora_exclusion_arts='false'">
<xsl:if test="$deptos_excluidos='true'">
	arts_a_bonificar = purchase meet (0.00001 * !(<xsl:value-of select="$deptos"/>));
</xsl:if>
<xsl:if test="$deptos_excluidos='false'">
	arts_a_bonificar = purchase;
</xsl:if>
</xsl:if>
...
//********** Promoción Invel: Importe Total Plan de Pago Deptos Excluidos // Impresiones en el Ticket  *************

Promotion postpago incondicional_plan_pago_deptos_excluidos_LOGS_<xsl:value-of select="$cod_promo"/>
...
<!-- <<<<<<<<<<<<<< -->
	// Parametros correspondientes a la promo incondicional_plan_pago_deptos_excluidos_<xsl:value-of select="$cod_promo"/>
	extern cantidad_<xsl:value-of select="$index"/>;
	extern publicar_arts_<xsl:value-of select="$index"/>;

	extern promo_activada_<xsl:value-of select="$index"/>;
	// -------------------------------- promo incondicional_plan_pago_deptos_excluidos_<xsl:value-of select="$cod_promo"/>
<!-- >>>>>>>>>>>>>> -->
</xsl:if>
...
<!-- <<<<<<<<<<<<<< -->
	// Aplicando Beneficios correspondientes a la promo incondicional_plan_pago_deptos_excluidos_<xsl:value-of select="$cod_promo"/>
