...
<!--  Autor2: Marcelo mancuso 21/09/2015                                -->
...
# Condiciones de Vigencia.
		<xsl:apply-templates select="preconditions">
			...
			<xsl:with-param name="promo_detalle"><xsl:for-each select="detalle/linea"><xsl:apply-templates/>\n</xsl:for-each></xsl:with-param>
...
#		<xsl:if test="por_cada_arts"> 
			<xsl:apply-templates select="por_cada_arts">
#				<xsl:with-param name="promo_id"><xsl:value-of select="@id"/></xsl:with-param>
			</xsl:apply-templates>
#		</xsl:if>
		<xsl:variable name="trporcadaart">
			<xsl:choose>
				<xsl:when test="$val_rangofecha='true'">Aca va el tipo</xsl:when>
				<xsl:when test="$val_rangofecha='false'"><xsl:apply-templates select="por_cada_arts"></xsl:apply-templates></xsl:when>
			</xsl:choose>
		</xsl:variable>
...
<xsl:param name="por_cada_arts"></xsl:param>
...
<xsl:variable name="vvigencia">
	<xsl:choose>
		...
		<xsl:when test="$val_vigencia='false'">"0"</xsl:when>
	</xsl:choose>
</xsl:variable>
...
<xsl:variable name="vperfil">
	<xsl:choose>
		...
		<xsl:when test="$val_perfil='false'">"0"</xsl:when>
	</xsl:choose>
</xsl:variable>
...
<xsl:variable name="trporcadaart">
	<xsl:choose>
		<xsl:when test="$val_rangofecha='true'">Aca va el tipo</xsl:when>
		<xsl:when test="$val_rangofecha='false'"><xsl:value-of select="$por_cada_arts"/>; </xsl:when>
	</xsl:choose>
</xsl:variable>
...
<!-- MMAncuso para las promox mxn  -->
<!-- 6;1;100;100;PROMOCION 6X5 CERVEZAS; -->
<xsl:variable name="trtMXN">
	<xsl:choose>
		<xsl:when test="$val_rangofecha='true'">6;1;100;100;PROMOCION 6X5 CERVEZAST1 <xsl:value-of select="$trporcadaart"/></xsl:when>
	    <xsl:when test="$val_rangofecha='false'">B6;1;100;100;PROMOCION 6X5 CERVEZAST1 <xsl:value-of select="trporcadaart"/></xsl:when>
			<xsl:apply-templates select="bonifico_arts"></xsl:apply-templates>
		</xsl:when>	
	</xsl:choose>	
</xsl:variable>
-->
<!-- <xsl:value-of select="$por_cada_arts"/>; bonifico_arts ;  descuento;100;name     -->
